...
public class UserDaoJdbcImpl implements UsersDao{
    ...
    private final static String SQL_INSERT_INTO_USERS =
            "INSERT INTO users (nickname, password, firstname, surname, avatar, email, role) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?);";

    private final static String SQL_FIND_NICKNAME_IGNORE_CASE =
            "SELECT * FROM users WHERE UPPER(nickname) = ?;";
    ...
    public void save(User user) {
        try {
            con = getConnection();
            ptmt = con.prepareStatement(SQL_INSERT_INTO_USERS);
    ...
    public User findByNicknameIgnoreCase(String name) {
        ...
        try {
            con = getConnection();
            ptmt = con.prepareStatement(SQL_FIND_NICKNAME_IGNORE_CASE);
